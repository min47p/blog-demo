...
이제 모델을 구체적으로 정의하기 위해 **매개변수(parameter)** $\phi$를 도입하여, $\phi$의 값에 따라 밀도함수의 형태가 결정되도록 하자. 이렇게 매개화된 확률 분포를 $p_{\phi}$로 표기한다. 이제 우리의 목표는 $p_{\phi}$가 $p_{\mathrm{data}}$에 최대한 가까워지도록 하는 $\phi$를 찾는 것이 된다. $p_{\phi}$의 형태에 제한을 두었기 때문에, 이렇게 찾은 $p_{\phi}$는 $p_{\mathrm{data}}$의 형태를 충분히 표현하지 못할 수도 있다. 하지만 우리가 허용한 형태 중 실제 데이터를 가장 잘 설명하는 분포를 찾은 것이기 때문에, 충분히 유용하다.
...
경사 하강법은 현재 매개변수 $\phi$에서 목적 함수의 gradient인 $\nabla_{\phi} J(\phi)$를 계산한 뒤, gradient의 반대 방향으로 $\phi$를 조금씩 업데이트하는 방법이다.
...
사실 우리는 식 {{< eqref gradient-descent >}}을 적용한 경사 하강법으로 최적화 문제를 풀어야 하므로, 식 {{< eqref kl-objective >}}의 기댓값 자체가 아니라 $\phi$에 대한 gradient가 필요하다. 다행히, 이 값도 마찬가지로 몬테 카를로 근사를 통해 구할 수 있다. 적절한 조건 아래에서는 $\nabla$ (미분)과 $\mathbb{E}$ (적분)을 교환할 수 있기 때문이다. 이 조건은 딥러닝에서는 보통 성립한다.
...
$$
\begin{align*}
p_{\phi}(\mathbf{x}) &= p_{\phi}(x_{1}) \prod_{i=2}^{L} p_{\phi}(x_{i} \mid x_{1}, \cdots, x_{i - 1})\\
&= p_{\phi}(x_{1}) \prod_{i=2}^{L} p_{\phi}(x_{i} \mid x_{1:i-1})
\end{align*}
$$
...
따라서 $p_{\phi}(x_{1})$, $\cdots$, $p_{\phi}(x_{L} \mid x_{1:L-1})$을 모두 단순한 분포로 만들면 문제가 해결된다.
이러한 아이디어는 $L$이 고정되어 있지 않을 때에도 사용할 수 있다.

Autoregressive generation은 텍스트 생성에 특히 적합하다. 텍스트를 토큰(token) 단위로 분해하면, 각 토큰은 $K$개의 고정된 vocabulary 중 하나이다 ($K$는 보통 수만에서 수십만 정도이다). $i$번째 토큰을 $x_{i}$로 놓으면, $p_{\phi}(x_{i} \mid x_{1:i-1})$는 $K$개의 값 중 하나를 선택하는 범주형 분포가 된다. 이 분포를 구체적으로 모델링해 보자.

$f_{\phi}(x_{1:i-1})$을 이전 토큰들을 입력으로 받아 $K$차원의 실수 벡터를 출력하는 신경망이라 하고, 그 $k$번째 원소를 $f_{\phi}(x_{1:i-1})_{k}$로 표기하자. 이제 식 {{< eqref normalized-distribution >}}에서와 같이 '0 이상' 조건과 정규화 조건을 고려하면, 확률 분포 $p_{\phi}(x_{i} \mid x_{1:i-1})$를 다음과 같이 정의할 수 있다.

$$p_{\phi}(x_{i} = k \mid x_{1:i-1}) = \frac{\exp(f_{\phi}(x_{1:i-1})_{k})}{\sum_{j=1}^{K} \exp(f_{\phi}(x_{1:i-1})_{j})}$$

식 {{< eqref normalized-distribution >}}에서 정규화를 위해 다루기 어려운 고차원 적분이 필요했던 것과 달리, 여기에서의 분모는 $K$개 항의 유한한 합이므로 쉽게 계산할 수 있다. $\phi$에 대한 미분도 가능하므로 학습 가능성 조건도 만족한다. 샘플링 역시 범주형 분포에서 하는 것이므로 간단하다.
...
여기에서 $p(\mathbf{z})$는 잠재 변수의 사전 분포이고, $p_{\phi}(\mathbf{x} \mid \mathbf{z})$는 잠재 변수가 주어졌을 때 데이터의 조건부 분포이다. 이 두 분포는 일반적으로 단순한 분포가 되도록 정의한다. 예를 들어, $p(\mathbf{z})$를 표준 정규 분포로, $p_{\phi}(\mathbf{x} \mid \mathbf{z})$를 $\mathbf{z}$에 의해 평균과 분산이 결정되는 정규 분포로 설정할 수 있다. 즉, $\boldsymbol{\mu}_{\phi}(\mathbf{z})$와 $\boldsymbol{\Sigma}_{\phi}(\mathbf{z})$가 결정론적인 함수일 때, 다음과 같이 정의할 수 있다.
...
{{< callout type="Idea" >}}
정규화된 밀도 함수 $p_{\phi}(\mathbf{x})$를 명시적으로 구하지 않은 상태에서 학습(최적화)도 할 수 있고, 샘플링도 수 있는 방법을 찾자.
{{< /callout >}}

놀랍게도 이러한 방법이 존재한다. 바로 무작위 알고리즘 중 하나인 Markov chain Monte Carlo (MCMC) 를 이용하는 것이다. 이러한 방식은 score-based model을 다룰 때 더 자세히 살펴보자.

{{< callout type="Note" >}}
Idea 2에서 등장한 GAN도 $p_{\phi}(\mathbf{x})$를 명시적으로 구하지 않은 상태에서 학습하는 모델이다. 단, 이 모델은 latent variable을 사용하므로 정규화는 자동으로 된다.
{{< /callout >}}

# 정리

이 글에서는 생성 모델이 풀고자 하는 문제를 수학적으로 모델링해 보았다. 생성 문제는 실제 데이터의 확률 분포 $p_{\mathrm{data}}$에 가까운 분포 $p_{\phi}$를 찾고, 이 분포에서 샘플링하는 문제가 된다. 여기에서 $\phi$는 분포를 결정하는 매개변수이고, 적절한 $\phi$를 찾는 것은 최적화 문제가 된다. 최적화 문제를 풀기 위해서는 목적 함수의 gradient를 구할 수 있어야 한다. 목적 함수에 기댓값이 포함되어 있다면 몬테 카를로 근사를 적용할 수 있어야 한다.

$p_{\phi}$를 구현할 때는 표현력, 정규화, 학습 가능성, 샘플링 가능성이라는 네 가지 조건을 고려해야 한다. 균등 분포, 정규 분포, 범주형 분포처럼 단순한 분포들을 재료로 하고 신경망을 활용해 표현력 높은 복잡한 분포를 만들어야 한다. 특히 정규화와 샘플링 가능성을 만족시키기가 어려운데, 이를 해결하기 위한 세 가지 핵심 아이디어를 살펴보았다.
