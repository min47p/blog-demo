...
$$p(\mathbf{x}) \approx p_{\mathrm{data}}(\mathbf{x}) \tag{1}$$
를 만족하는 $p(\mathbf{x})$를 찾고, $\mathbf{x} \sim p(\mathbf{x})$를 수행하는 문제가 된다.

조건 $\mathbf{c}$가 주어지는 경우에는 조건부 확률을 도입해 마찬가지로 모델링할 수 있다. 이 경우
$$p(\mathbf{x} \mid \mathbf{c}) \approx p_{\mathrm{data}}(\mathbf{x} \mid \mathbf{c}) \tag{2}$$
...
$$p(\mathbf{x}_{i}) = \frac{1}{N} \; , \qquad i = 1, \cdots , N \tag{3}$$
...
$$p_{\phi}(\mathbf{x}) = \frac{1}{(2\pi)^{d/2} |\Sigma|^{1/2}} \exp\left( -\frac{1}{2} (\mathbf{x} - \mu)^{\top} \Sigma^{-1} (\mathbf{x} - \mu) \right) \tag{4}$$
{{< /callout >}}
...
$$\mathcal{L}(\phi \mid \mathbf{x}_{i}) = p_{\phi}(\mathbf{x}_{i}) \tag{5}$$

$N$개의 샘플이 $p_{\mathrm{data}}$로부터 얻은 IID 샘플이므로, $N$개의 샘플 전체가 주어졌을 때 $\phi$의 가능도를 다음과 같이 정의할 수 있다. 독립성에 의해 각각의 샘플에 의한 가능도의 곱으로 나타난다.
$$\mathcal{L}(\phi \mid \mathbf{x}_{1}, \cdots, \mathbf{x}_{N}) = \prod_{i} p_{\phi} (\mathbf{x}_{i}) = \prod_{i} \mathcal{L}(\phi \mid \mathbf{x}_{i}) \tag{6}$$
...
$$
p(\phi \mid \mathbf{x}_{1}, \cdots, \mathbf{x}_{N}) = \frac{p(\mathbf{x}_{1}, \cdots, \mathbf{x}_{N} \mid \phi) \;p(\phi)}{\int p(\mathbf{x}_{1}, \cdots, \mathbf{x}_{N} \mid \phi) \;p(\phi) \; d\phi} \tag{9}
$$
...
이 항을 조금 더 살펴보자. $d$ 개의 원소로 이루어진 매개 변수 $\phi$가 정규 분포를 따르고 각 원소의 분산이 $\sigma^{2}$이라 가정하면 (즉, $\phi \sim \mathcal{N}(0, \sigma^{2} I)$라면), 밀도함수 $p(\phi)$는 다음과 같다.

$$p(\phi) = \frac{1}{(2\pi\sigma^{2})^{d/2}} \exp\left( -\frac{\|\phi\|^2}{2\sigma^{2}} \right) \tag{11}$$

$\log p(\phi)$는 다음과 같다.

$$\log p(\phi) = - \frac{\|\phi\|^2}{2\sigma^{2}} - \frac{d}{2} \log(2\pi\sigma^{2}) \tag{12}$$
...
$$D_{\mathrm{KL}}(p \| q) = \mathbb{E}_{\mathbf{x} \sim p(\mathbf{x})} \left[\log\frac{p(\mathbf{x})}{q(\mathbf{x})}\right] \tag{13}$$
...
$$\phi = \argmax_{\phi} \; \frac{1}{N} \sum_{i=1}^{N} \log p_{\phi}(\mathbf{x}_{i}) \tag{16}$$
...
$$D_{f}(p \| q) = \mathbb{E}_{\mathbf{x} \sim q(\mathbf{x})} \left[ f\left( \frac{p(\mathbf{x})}{q(\mathbf{x})} \right) \right] \tag{17}$$
...
최적화 문제를 푸는 방법과 관련해서 언급할 내용이 한 가지 더 있다. 딥러닝에서의 목적 함수는 식 $(15)$와 같이 기댓값을 포함하고 있는 경우가 많다. 일반적으로 $\mathbb{E}_{\mathbf{x} \sim p(\mathbf{x})} [f(\mathbf{x})]$와 같은 식의 값을 정확히 계산하는 것은 $p$와 $f$가 단순하지 않은 이상 불가능하다. 왜냐하면 다음과 같은 복잡한 적분 형태로 표현되기 때문이다.
$$\mathbb{E}_{\mathbf{x} \sim p(\mathbf{x})} [f(\mathbf{x})] = \int f(\mathbf{x}) p(\mathbf{x})\; d\mathbf{x} \tag{21}$$
하지만 $p(\mathbf{x})$에서 샘플 $\mathbf{x}_{1}$, $\cdots$, $\mathbf{x}_{N}$을 추출할 수 있다면, 기댓값을 다음과 같이 근사할 수 있다.

$$\mathbb{E}_{\mathbf{x} \sim p(\mathbf{x})} [f(\mathbf{x})] \approx \frac{1}{N} \sum_{i=1}^{N} f(\mathbf{x}_{i}), \quad \mathbf{x}_{i} \sim p(\mathbf{x}) \tag{22}$$

이러한 방법을 몬테 카를로 근사(Monte Carlo approximation)라 한다. 큰 수의 법칙(LLN, Law of Large Numbers)에 의해 $N \to \infty$일 때 우변이 좌변에 수렴하는 것이 보장된다. 앞에서 식 $(16)$을 식 $(15)$의 기댓값을 $N$개의 샘플로 근사하여 얻은 것도 같은 원리이다.

사실 우리는 식 $(20)$을 적용한 경사 하강법으로 최적화 문제를 풀어야 하므로, 식 $(15)$의 기댓값 자체가 아니라 그것을 $\phi$로 미분한 결과가 필요하다. 다행히, 이 값도 마찬가지로 몬테 카를로 근사를 통해 구할 수 있다.

$$\nabla_{\phi} \mathbb{E}_{\mathbf{x} \sim p_{\mathrm{data}}(\mathbf{x})} [\log p_{\phi}(\mathbf{x})] = \mathbb{E}_{\mathbf{x} \sim p_{\mathrm{data}}(\mathbf{x})} [\nabla_{\phi} \log p_{\phi}(\mathbf{x})] \approx \frac{1}{N} \sum_{i=1}^{N} \nabla_{\phi} \log p_{\phi}(\mathbf{x}_{i}) \tag{23}$$
...
3. **학습 가능성**: 최적화 문제를 풀 수 있어야 한다. 앞 절에서 이야기한 것처럼 목적 함수를 $\phi$로 미분할 수 있어야 하고, 기댓값이 있는 경우 몬테 카를로 근사를 적용할 수 있어야 한다.
...
$$
\int C \exp (f_{\phi}(\mathbf{x})) \;d\mathbf{x} = 1 \quad \Rightarrow \quad C = \left(\int \exp(f_{\phi}(\mathbf{x})) \;d\mathbf{x}\right)^{-1} \tag{24}
$$

결과적으로, 우리는 다음과 같이 정규화 조건을 만족하는 확률 분포를 얻을 수 있다.
$$
p_{\phi}(\mathbf{x}) = C \exp(f_{\phi}(\mathbf{x})) = \frac{\exp(f_{\phi}(\mathbf{x}))}{\int \exp(f_{\phi}(\mathbf{x})) \;d\mathbf{x}} \tag{25}
$$
...
{{< callout type="Note" >}}
앞의 식 $(21)$의 복잡한 적분은 몬테 카를로로 잘 근사하지 않았나? 라는 의문이 들 수 있다. 하지만 식 $(21)$의 적분은 확률 분포 $p(\mathbf{x})$에 대한 기댓값이고, $p(\mathbf{x})$에서 샘플링이 가능했기 때문에 몬테 카를로 근사를 적용할 수 있었다. 반면 정규화 상수의 적분 $\int \exp(f_{\phi}(\mathbf{x})) \, d\mathbf{x}$는 우리가 샘플링할 수 있는 확률 분포에 대한 기댓값으로 자연스럽게 표현되지 않으므로, 몬테 카를로 근사를 직접 적용할 수 없다.
{{< /callout >}}
...
$$p_{\phi}(\mathbf{x}) = \prod_{i} p_{\phi}(x_{i} \mid x_{1}, \cdots, x_{i - 1}) = \prod_{i} p_{\phi}(x_{i} \mid x_{1:i-1}) \tag{26}$$
...
$$p_{\phi}(x_{i} = k \mid x_{1:i-1}) = \frac{\exp(f_{\phi}(x_{1:i-1})_{k})}{\sum_{j=1}^{K} \exp(f_{\phi}(x_{1:i-1})_{j})} \tag{27}$$
...
$$p_{\phi}(\mathbf{x}) = \int p_{\phi}(\mathbf{x}, \mathbf{z}) \, d\mathbf{z} = \int p_{\phi}(\mathbf{x} \mid \mathbf{z}) \, p(\mathbf{z}) \, d\mathbf{z} \tag{28}$$
...
$$p(\mathbf{z}) = \mathcal{N}(\mathbf{0}, \mathbf{I}), \qquad p_{\phi}(\mathbf{x} \mid \mathbf{z}) = \mathcal{N}(\boldsymbol{\mu}_{\phi}(\mathbf{z}), \boldsymbol{\Sigma}_{\phi}(\mathbf{z})) \tag{29}$$
...
$$\int p_{\phi}(\mathbf{x}) \, d\mathbf{x} = \int \left( \int p_{\phi}(\mathbf{x} \mid \mathbf{z}) \, p(\mathbf{z}) \, d\mathbf{z} \right) d\mathbf{x} = \int p(\mathbf{z}) \underbrace{\left( \int p_{\phi}(\mathbf{x} \mid \mathbf{z}) \, d\mathbf{x} \right)}_{= 1} d\mathbf{z} = \int p(\mathbf{z}) \, d\mathbf{z} = 1 \tag{30}$$
