...
{{< callout type="주의" >}}
$p(\mathbf{x})$라는 기호를 '$\mathbf{x}$가 따르는 확률 분포'라는 의미와 '이 확률 분포의 $\mathbf{x}$에서의 밀도함수 값'이라는 의미로 모두 사용할 것이다. 밀도함수를 별개의 함수로 나타내는 것이 더 정확하지만, 이렇게 혼용해 표기하는 것이 더 편하다.
{{< /callout >}}
...
사실 우리는 식 {{< eqref gradient-descent >}}을 적용한 경사 하강법으로 최적화 문제를 풀어야 하므로, 식 {{< eqref kl-objective >}}의 기댓값 자체가 아니라 $\phi$에 대한 gradient가 필요하다. 다행히, 이 값도 마찬가지로 몬테 카를로 근사를 통해 구할 수 있다.
