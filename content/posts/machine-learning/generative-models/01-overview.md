...
$p_{\theta}$를 구현할 때는 표현력, 정규화, 학습 가능성, 샘플링 가능성이라는 네 가지 조건을 고려해야 한다. 균등 분포, 정규 분포, 범주형 분포처럼 단순한 분포들을 재료로 하고 신경망을 활용해 표현력 높은 복잡한 분포를 만들어야 한다. 특히 정규화와 샘플링 가능성을 만족시키기가 어려운데, 이를 해결하기 위한 세 가지 핵심 아이디어를 살펴보았다.
