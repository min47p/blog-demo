...
실제 데이터 $\mathbf{x}$의 확률 분포 $p_{\mathrm{data}}({\mathbf{x}})$는 연쇄 법칙에 의해 다음과 같이 $L$개의 조건부 확률 분포들의 곱으로 표현할 수 있다. $x_{1}, \cdots, x_{i - 1}$을 $x_{1:i-1}$로 간단하게 표현했다. 또한, 편의상 $p_{\phi}(x_{1})$을 $p_{\phi}(x_{1} \mid x_{1:0})$으로 표기했다.
...
이를 이용하면, 매개화된 확률 분포 $p_{\phi}(\mathbf{x})$를 다음과 같이 구성할 수 있다.

{{< eqlabel p_phi-conditional >}}
$$p_{\phi}(\mathbf{x}) = \prod_{i=1}^{L} p_{\phi}(x_{i} \mid x_{1:i-1})$$
...
- $x_{1} \sim p_{\phi}(x_{1} \mid x_{1:0})$을 샘플링한다.
...
이제 autoregressive model을 어떻게 학습시키는지 알아보기 위해, $p_{\mathrm{data}}$와 $p_{\phi}$의 KL divergence를 최소화하는 문제를 생각해 보자. [이전 포스트](../01-overview)에서 보인 것처럼 이는 expected log-likelihood를 최대화하는 것, 즉 negative log-likelihood를 최소화하는 것과 동치이다. 따라서 목적 함수를 다음과 같이 정의하자.

$$
\begin{align*}
\phi &= \argmin_{\phi} J(\phi)\\
J(\phi) &= -\mathbb{E}_{\mathrm{x} \sim p_{\mathrm{data}}} [\log p_{\phi}(\mathbf{x})]
\end{align*}
$$

식 {{< eqref p_phi-conditional >}}을 이용해 $J(\phi)$를 정리하면 다음과 같다.

$$
\begin{align*}
J(\phi) &= -\mathbb{E}_{\mathrm{x} \sim p_{\mathrm{data}}} \left[\log \prod_{i=1}^{L} p_{\phi}(x_{i} \mid x_{1:i-1})\right]\\
&= -\mathbb{E}_{\mathrm{x} \sim p_{\mathrm{data}}} \left[ \sum_{i=1}^{L} \log p_{\phi}(x_{i} \mid x_{1:i-1})\right]
\end{align*}
$$

$p_{\phi}(x_{i} \mid x_{1:i-1})$을 모두 단순한 분포로 설정했으므로, 이들의 밀도 함수나 $\phi$에 대한 gradient를 쉽게 구할 수 있다. 이제 목적 함수 $J(\phi)$의 gradient를 계산할 수 있는지 살펴보자.

$$
\begin{align*}
\nabla_{\phi} J(\phi) &= -\mathbb{E}_{\mathrm{x} \sim p_{\mathrm{data}}} \left[ \sum_{i=1}^{L} \nabla_{\phi} \log p_{\phi}(x_{i} \mid x_{1:i-1})\right]
\end{align*}
$$
...
$$
\nabla_{\phi} J(\phi) \approx -\frac{1}{N} \sum_{n=1}^{N} \sum_{i=1}^{L} \nabla_{\phi} \log p_{\phi}(x_{i}^{(n)} \mid x_{1:i-1}^{(n)})
$$

위 식은 계산 가능하므로, 우리는 $J(\phi)$에 대한 최적화 문제를 경사 하강법으로 풀 수 있다. 위 식에서는 데이터의 길이가 $L$로 고정되어 있지만, 데이터의 길이가 고정되지 않은 경우에 대해서도 마찬가지로 할 수 있다.
...
텍스트를 토큰(token) 단위로 분해하면, 각 토큰은 $K$개의 고정된 vocabulary 중 하나이다 ($K$는 보통 수만에서 수십만 정도이다). $i$번째 토큰을 $x_{i}$로 놓으면, $p_{\phi}(x_{i} \mid x_{1:i-1})$는 $K$개의 값 중 하나를 선택하는 범주형 분포가 된다.

이 범주형 분포를 구체적으로 모델링해 보자. $f_{\phi}(x_{1:i-1})$을 이전 토큰들을 입력으로 받아 $K$차원의 실수 벡터를 출력하는 신경망이라 하고, 출력 벡터의 $k$번째 원소를 $f_{\phi}(x_{1:i-1})_{k}$로 표기하자. 이들을 logit이라 한다. '0 이상' 조건과 정규화 조건을 만족시키기 위해 logit에 [softmax 함수](https://en.wikipedia.org/wiki/Softmax_function)를 적용하면 다음과 같이 범주형 분포의 확률 분포를 얻을 수 있다.
...
$$-\nabla_{\phi} \log p_{\phi}(x^{(n)}_{i} \mid x^{(n)}_{1:i-1}) = -\nabla_{\phi}  f_{\phi}(x^{(n)}_{1:i-1})_{x^{(n)}_{i}} + \nabla_{\phi} \log \sum_{j=1}^{K} \exp(f_{\phi}(x^{(n)}_{1:i-1})_{j})$$

첨자가 많아 헷갈리기는 하지만, 아무튼 위 식은 쉽게 계산할 수 있는 식이다.

{{< callout type="Note" >}}
위에서는 목적 함수 $J(\phi)$의 gradient에 대한 식을 쓴 다음 몬테 카를로 근사를 적용했다. 학습을 위해 필요한 값이 $J(\phi)$가 아니라 $\nabla_{\phi} J(\phi)$이기 때문에 그렇게 한 것이다.

이번에는 목적 함수의 의미를 알아보기 위해, 목적 함수에 바로 몬테 카를로 근사를 적용해 $J(\phi)$를 계산 가능한 형태로 정리해 보자.

$$
\begin{align*}
J(\phi) &= -\mathbb{E}_{\mathrm{x} \sim p_{\mathrm{data}}} \left[ \sum_{i=1}^{L}\log p_{\phi}(x_{i} \mid x_{1:i-1})\right]\\
&\approx \frac{1}{N} \sum_{n=1}^{N} \sum_{i=1}^{L} \left[ -\log p_{\phi}(x^{(n)}_{i} \mid x^{(n)}_{1:i-1}) \right]
\end{align*}
$$

하나의 샘플 $\mathbf{x}^{(n)}$만 고려하자. 식 {{< eqref softmax-categorical >}}을 대입하면 $i$번째 항은 다음과 같다.

$$-\log p_{\phi}(x^{(n)}_{i} \mid x^{(n)}_{1:i-1}) = -f_{\phi}(x^{(n)}_{1:i-1})_{x^{(n)}_{i}} + \log \sum_{j=1}^{K} \exp(f_{\phi}(x^{(n)}_{1:i-1})_{j})$$
...
학습 과정에서 주목할 점이 하나 있다. 샘플링 과정에서는 $x_{1}$부터 $x_{L}$까지 순차적으로 생성해야 하지만, 학습 과정에서는 정답 토큰 $x_{1}^{(n)}$, $\cdots$, $x_{L}^{(n)}$이 이미 주어져 있다. 따라서 $f_{\phi}(x_{1:i-1}^{(n)})$를 모든 $i$에 대해 동시에 계산할 수 있다. 이처럼 학습 시 정답 데이터를 신경망의 입력으로 사용하는 방식을 **teacher forcing**이라 한다. Teacher forcing 덕분에 학습은 효율적으로 병렬화할 수 있지만, 샘플링(생성)은 여전히 순차적으로 수행해야 한다.
...
- **신경망 구조**: $f_{\phi}$를 어떤 신경망으로 구현할 것인가?
